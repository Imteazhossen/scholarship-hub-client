...
import { FaMapMarkerAlt, FaCalendarAlt, FaDollarSign, FaStar, FaSearch, FaSortAmountDown, FaSortAmountUp } from 'react-icons/fa';
          ...
          <>
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {pageData.map(s => {
                ...
                return (
                  <article key={s._id} className="bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-2xl transform hover:-translate-y-1 transition">
                    <div className="relative h-40 overflow-hidden">
                      <img src={s.logoURL} alt={s.universityName} className="object-cover w-full h-full" />
                    </div>
                    <div className="p-5 flex flex-col justify-between h-[260px]">
                      <div className="space-y-1">
                        <h3 className="text-xl font-semibold text-gray-800">{s.universityName}</h3>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">({rating.toFixed(1)})</p>
                        <NavLink
            ...
            {pageCount > 1 && (
              <div className="flex justify-center items-center space-x-2 mt-12">
                <button onClick={() => setPage(p => Math.max(p - 1, 1))} disabled={page === 1} className="btn btn-sm">
                  Prev
                </button>
                ...
                <button onClick={() => setPage(p => Math.min(p + 1, pageCount))} disabled={page === pageCount} className="btn btn-sm">
                  Next
                </button>
              </div>
            )}
