...
export default function AddScholarship() {
  ...
  const onSubmit = async data => {
    try {
      ...
      const uploadRes = await api.post(uploadUrl, form, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      const logoURL = uploadRes.data.data.url;

      // 2) Build scholarship object
      const scholarship = {
        name:               data.name,
        ...
        applicationDeadline:data.applicationDeadline,  // e.g. "2025-08-31"
        postDate:           data.postDate,             // e.g. "2025-07-24"
        postedBy:           user.email,
      };
  ...
  return (
    <div className="min-h-screen rounded-xl p-6 bg-gradient-to-r from-pink-50 via-sky-100 to-emerald-50">
